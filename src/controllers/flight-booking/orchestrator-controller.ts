...
//       flightNumber,
//       tripType,
//       expectedFare, // Optional: for fare validation if provided
//       passengers,
//       contact,
//     } = req.body;
...
//       if (!itinerary) throw new Error("No Same flight found");
...
//       if(adtAmounts >= expectedFare && expectedFare) {
//         throw new Error(
//           `Expected fare ${expectedFare} does not match ADT fare ${adtAmounts?.GrossAmount}`
//         );
//       }
//       totalBaseAmount =
...
function floatEq(a: number, b: number, tol = 2) {
  // tol = decimal places
  return Math.abs(a - b) < Math.pow(10, -tol);
}
...
export const oneClickBooking = async (
  ...
  try {
    ...
    const {
      ...
      flightNumber,
      tripType = "D",
      expectedFare, // expected gross fare per ADT
      passengers,
      contact,
    } = req.body;
    ...
    try {
      ...
      // Require EXPLICIT flight number match!
      itinerary =
        searchData?.ItineraryFlightList?.[0]?.Items?.find(
          (item) =>
            flightNumber &&
            item.FlightDetails?.[0]?.FlightNumber?.replace(/\s/g, "") ===
              flightNumber.replace(/\s/g, "")
        );
      if (!itinerary) throw new Error("No matching flight number found in availability!");
      searchTrackid = searchData?.Trackid;
      progress.steps.push({
        step: "search",
        status: "success",
    ...
    try {
      ...
      // Fare check logic:
      if (expectedFare) {
        // total expected fare for ADT passengers
        const expectedTotal =
          parseFloat(expectedFare) * (paxCounts.ADT > 0 ? paxCounts.ADT : 1);
        const actualTotal =
          amountOr0(adtAmounts?.GrossAmount) * (paxCounts.ADT > 0 ? paxCounts.ADT : 1);

        if (actualTotal > expectedTotal + 0.01) {
          throw new Error(
            `Fare changed! Expected total for ${paxCounts.ADT} adult(s): ₹${expectedTotal}, but found: ₹${actualTotal}`
          );
        }
      }
